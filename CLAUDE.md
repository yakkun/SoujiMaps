...
Leaflet 1.9.4 is loaded from unpkg via `<script>`/`<link>` tags in `index.html` with SRI hashes. JSZip 3.10.1 is loaded the same way — it is used only by the KMZ path in the file-drop pipeline. MapLibre GL 4.7.1 plus the `@maplibre/maplibre-gl-leaflet` 0.0.22 plugin are also loaded there, to render the vector-tile basemaps (地理院地図Vector / れきちず). To bump any of these libraries, update both the URL and the `integrity` attribute.
...
`LEFT_LAYERS` and `RIGHT_LAYERS` are arrays at the top of `main.js`. Left entries are `{ name, build }`; right entries also carry `linkHref` / `linkText` so the right pane header's external link + `aria-label` can be updated whenever the user switches basemaps, plus a `group` label that drives `<optgroup>` rendering in the header `<select>` and an optional `requires` field that gates API-key-only providers. `LEFT_LAYERS` currently uses only index `0`; `RIGHT_LAYERS` defines up to ~38 presets across 7 groups, any of which may be hidden if their API key is missing (or MapLibre failed to load): **標準地図** (OSM, OSM Humanitarian, CARTO Voyager, Esri ストリート), **淡色・ダーク** (CARTO Positron/Dark Matter, Esri グレーキャンバス, Stadia Alidade Smooth Dark *(key)*, Stamen Toner *(key)*), **Google Maps** (道路/地形/航空写真/ハイブリッド), **地理院 (日本)** (標準/淡色/白地図/色別標高図 — raster), **地形・アウトドア** (OpenTopoMap, Esri 地形図, Esri ナショジオ, CyclOSM, Esri 海洋, Stadia Outdoors *(key)*, Stamen Terrain *(key)*, Thunderforest Outdoors/Landscape *(key)*, MapTiler Outdoor/Winter *(key)*, Mapbox Outdoors *(key)*), **衛星・空中写真** (地理院 空中写真, Esri 衛星写真, Mapbox 衛星ストリート *(key)*), and **ベクター (日本)** (地理院ベクター 標準/淡色/白地図, れきちず 江戸/ひらがな/English — all *(MapLibre)*). `(key)` marks entries gated by `requires`; `(MapLibre)` marks entries gated on `HAS_MAPLIBRE` (MapLibre GL JS loading successfully).
...
`buildGoogleTileLayer(lyrs)` (`m`=道路, `s`=航空, `y`=ハイブリッド, `p`=地形) and `buildEsriTileLayer(service, { maxNativeZoom, source })` are shared factories — service names come from the `server.arcgisonline.com/ArcGIS/rest/services/<service>/MapServer` path, and each Esri service has its own attribution "source" string plus (for some) a lower `maxNativeZoom` than Leaflet's default. `buildMaplibreLayer(styleUrl, attribution)` wraps `L.maplibreGL({ style, attribution })` from the `@maplibre/maplibre-gl-leaflet` plugin, so vector-tile styles (MapLibre Style Spec / Mapbox GL Style JSON) plug into the same `{ preset, layer }` pipeline as raster presets — switching between raster and vector is transparent to `populateRightBasemapSelect` and the change handler. The MapLibre canvas lives in Leaflet's `tilePane` (z-index 200), so the hillshade overlay's custom pane (`hillshadePane`, z-index 250) continues to composite above it with `mix-blend-mode: soft-light`. The MapLibre WebGL canvas is transparent by default, and not every style ships an opaque `background` layer — so `style.css` paints `.maplibregl-map` plain white (`#fff`) to stop the dark `.leaflet-container` (`#0b1220`) bleeding through during load or on styles without a background fill. Tweak this swatch if a new vector basemap looks washed out; do not remove it. **れきちず uses CC BY-NC-ND 4.0** — it's appropriate for this personal/non-commercial viewer, but a forker who wants to monetise or modify should route around it. Attribution is non-negotiable (`REKICHIZU_ATTR`).
